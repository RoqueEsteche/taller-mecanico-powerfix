/**
 * LandingPage.tsx — Stihl Motors
 *
 * Cumple los requisitos:
 *  - BEM CSS (.landing, .navbar, .hero, .product-card, .contact-form, etc.)
 *  - CSS Custom Properties en :root (paleta 60-30-10)
 *  - Event listeners sin atributos inline
 *  - Validación de formulario con feedback visual inmediato
 *  - Fetch asíncrono con async/await y try/catch
 *  - Web Storage: localStorage (tema, cookies) y sessionStorage (última sección)
...
const SERVICES = [
  { icon: '🔧', title: 'Reparación de Motosierras', desc: 'Servicio técnico autorizado para STIHL, Husqvarna, Echo y más. Diagnóstico digital incluido.' },
  { icon: '💧', title: 'Hidrolavadoras', desc: 'Revisión completa, cambio de sellos, bombas y accesorios Kärcher, Nilfisk y Husqvarna.' },
  { icon: '🌿', title: 'Desmalezadoras y Sopladores', desc: 'Afinación, cambio de cabezal y reparación de transmisión en todos los modelos del mercado.' },
  { icon: '⚡', title: 'Repuestos Originales', desc: 'Stock permanente para más de 234 modelos de 15 marcas. Entrega inmediata.' },
  ...
  { icon: '🖥️', title: 'Diagnóstico Digital', desc: 'Diagnóstico computarizado para equipos modernos con puerto de comunicación electrónica.' },
];

const EMPTY_FORM: ContactForm = { name: '', email: '', phone: '', service: '', message: '' };

// ── Icono SVG de herramienta (profesional) ────────────────────────────────────
function ToolIcon() {
  return (
    <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="hero__visual-svg">
      <path d="M42 10C36.48 10 32 14.48 32 20c0 1.18.21 2.31.58 3.36L12.7 43.24A6 6 0 0 0 17 54a6 6 0 0 0 4.24-10.24L41.36 23.7c1.05.37 2.18.58 3.36.58C50.2 24.28 54 20.48 54 15.28c0-1-.13-1.97-.36-2.9l-6.72 6.72-4.24-4.24 6.72-6.72A9.94 9.94 0 0 0 42 10z" fill="white" fillOpacity="0.9"/>
    </svg>
  );
}
...
export default function LandingPage({ onGoToAdmin }: { onGoToAdmin: () => void }) {
  ...
  // ── Cargar catálogo público desde la API ─────────────────────────────────────
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch('/api/public/parts');
  ...
  // ── Envío del formulario (async/await + fetch) ────────────────────────────────
  ...
  return (
    <div className="landing">
      ...
      <header className="navbar" role="banner">
        <div className="navbar__inner">
          ...
          <nav className="navbar__nav" aria-label="Navegación principal">
            ...
            <a href="/seguimiento" className="navbar__link">Seguir OT</a>
          </nav>
          ...
          <div className="navbar__actions">
            ...
            <button type="button" className="btn btn--primary" onClick={onGoToAdmin}>
              Panel Admin
            </button>
          </div>
        </div>
      ...
      <main role="main">
        ...
        <section id="inicio" className="hero" aria-labelledby="hero-heading">
          <div className="hero__inner">
            <div className="hero__content">
              <span className="hero__badge">Servicio Técnico Autorizado · Paraguay</span>
              <h1 id="hero-heading" className="hero__title">
                Expertos en
                <span className="hero__title--accent">Maquinaria de Potencia</span>
              </h1>
              <p className="hero__description">
                Más de 15 años brindando servicio técnico especializado en STIHL, Husqvarna,
                Kärcher y otras marcas líderes. Diagnóstico digital, repuestos originales
                y garantía en cada trabajo.
              </p>
            ...
            <div className="hero__image-box" aria-hidden="true">
              <div className="hero__visual">
                ...
                <span className="hero__visual-text">Stihl Motors</span>
                <span className="hero__visual-sub">Taller Especializado</span>
        ...
        {/* ── CATÁLOGO DINÁMICO (desde API pública) ─────────────────────────── */}
                ...
                ? Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="product-card product-card--skeleton" aria-hidden="true">
                      ...
                      <div className="product-card__body">
                        <div className="product-card__brand product-card__skeleton-line product-card__skeleton-line--short" />
                        <div className="product-card__title product-card__skeleton-line product-card__skeleton-line--long" />
                      </div>
                    </div>
                  ))
                : products.length === 0
                  ? (
                    <div className="catalog-empty">
                      ...
                      <p className="catalog-empty__title">Catálogo disponible en taller</p>
                      <p className="catalog-empty__text">
                        Contactanos directamente para consultar disponibilidad de repuestos.<br />
                        Atendemos más de 234 modelos de 15 marcas líderes.
                      </p>
                    </div>
                  )
                  : products.map(p => (
                    <article key={p.id} className="product-card">
                      ...
                      <div className="product-card__body">
                        <span className="product-card__brand">{p.machineBrand || 'Universal'}</span>
                        <h3 className="product-card__title">{p.description}</h3>
                        <span className="product-card__category">{p.machineCategory}</span>
            ...
            <div className="contact-section__inner">
              ...
              <div className="contact-info" aria-label="Información de contacto">
                <div className="contact-info__item">
                  ...
                  <div>
                    <p className="contact-info__label">Dirección</p>
                    <p className="contact-info__value">Asunción, Paraguay</p>
                  </div>
                </div>
                <div className="contact-info__item">
                  <div className="contact-info__icon" aria-hidden="true">📞</div>
                  <div>
                    <p className="contact-info__label">Teléfono / WhatsApp</p>
                    <p className="contact-info__value">+595 21 000-000</p>
                  </div>
                </div>
                <div className="contact-info__item">
                  <div className="contact-info__icon" aria-hidden="true">✉️</div>
                  <div>
                    <p className="contact-info__label">Correo electrónico</p>
                    <p className="contact-info__value">contacto@stihlmotors.com</p>
                  </div>
                </div>
                <div className="contact-info__item">
                  <div className="contact-info__icon" aria-hidden="true">🕐</div>
                  <div>
                    <p className="contact-info__label">Horario de atención</p>
                    <p className="contact-info__value">Lun–Vie 8:00 – 18:00<br />Sáb 8:00 – 12:00</p>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="contact-form__grid">
                  ...
                  {/* Dropdown cerrado — previene errores ortográficos */}
                  <div className="field">
          ...
          <div className="footer__grid">
            <div className="footer__brand">
              ...
              <p className="footer__tagline">
                ...
                Atendemos STIHL, Husqvarna, Kärcher, Honda y más de 15 marcas líderes del mercado.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="footer__links">
                ...
                <li><span className="footer__link">Honda · Makita · DeWalt</span></li>
              </ul>
            </div>

            <div>
              <p className="footer__col-title">Sistema</p>
              <ul className="footer__links">
      ...
      >
        <div className="cookie-banner__inner">
          ...
          <div className="cookie-banner__actions">
            <button type="button" className="btn btn--outline" onClick={rejectCookies}>
              Rechazar
            </button>
      ...
      >
        <div className="modal__box">
          <div className="modal__header">
            ...
            <button type="button" className="modal__close" onClick={() => setShowTerms(false)} aria-label="Cerrar">✕</button>
          </div>
          <div className="modal__body">
            ...
            <div className="modal__footer">
              <button type="button" className="btn btn--primary" onClick={() => setShowTerms(false)}>
                Entendido
              </button>
            </div>
          </div>
