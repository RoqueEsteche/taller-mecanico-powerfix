...
-- Columnas de pago en ventas (migración idempotente)
alter table sale add column if not exists payment_method text not null default 'cash';
alter table sale add column if not exists payment_ref text;

-- ── Usuarios del sistema (seed inicial) ──────────────────────────────────────
-- Contraseñas generadas con bcrypt (pgcrypto blowfish, costo 10)
-- admin@stihlmotors.com    → Admin2024!
-- recepcion@stihlmotors.com → Recep2024!
-- stock@stihlmotors.com    → Stock2024!
-- mecanico@stihlmotors.com → Meca2024!
insert into app_user (email, display_name, role, password_hash, active) values
  ('admin@stihlmotors.com',     'Administrador',     'admin',         crypt('Admin2024!',  gen_salt('bf', 10)), true),
  ('recepcion@stihlmotors.com', 'Ana Recepcionista', 'receiver',      crypt('Recep2024!',  gen_salt('bf', 10)), true),
  ('stock@stihlmotors.com',     'Carlos Stock',      'stock_manager', crypt('Stock2024!',  gen_salt('bf', 10)), true),
  ('mecanico@stihlmotors.com',  'Luis Mecánico',     'mechanic',      crypt('Meca2024!',   gen_salt('bf', 10)), true)
