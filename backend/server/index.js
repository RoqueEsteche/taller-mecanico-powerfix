...
async function ensureCatalogTables() {
  ...
  // Usuarios de demostración (solo si no existen)
  const adminExists = await query(`select id from app_user where email = 'admin@stihlmotors.com' limit 1`);
  if (adminExists.rowCount === 0) {
    const { hashPassword: hp } = await import('./auth.js');
    const users = [
      { email: 'admin@stihlmotors.com',       displayName: 'Administrador',    role: 'admin',         password: 'Admin2024!' },
      { email: 'recepcion@stihlmotors.com',    displayName: 'Ana Recepcionista', role: 'receiver',      password: 'Recep2024!' },
      { email: 'stock@stihlmotors.com',        displayName: 'Carlos Stock',      role: 'stock_manager', password: 'Stock2024!' },
      { email: 'mecanico@stihlmotors.com',     displayName: 'Luis Mecánico',     role: 'mechanic',      password: 'Meca2024!' },
    ];
    for (const u of users) {
      const hash = await hp(u.password);
      await query(
        `insert into app_user (email, display_name, role, password_hash, active)
         values ($1, $2, $3, $4, true)
         on conflict (email) do nothing`,
        [u.email, u.displayName, u.role, hash],
      );
    }
  }
}
...
// ── Catálogo público de repuestos (sin auth — para la landing page) ───────
app.get('/api/public/parts', async (_req, res) => {
  try {
    const result = await query(
      `select id, code, description, machine_category, machine_brand, machine_model, price, stock
       from part where stock > 0 order by machine_brand asc, description asc limit 30`,
    );
    ...
    return res.status(500).json({ message: 'No se pudo cargar el catálogo.' });
  }
});
